...
from app.models.recipe import Recipe, SwipeAction
...
def upsert_recipe_embedding(recipe: Recipe, ingredients: list):
    try:
            ...
            points=[
                models.PointStruct(
                    ...
                    payload={
                        ...
                        "household_id": str(recipe.household_id) if recipe.household_id else None,
                    },
                )
            ],
        )
...
def search_recipes_semantic(query: str, household_id: str | None = None, limit: int = 10):
    try:
        client = get_qdrant_client()
...
def get_recommendations(user_id: str, session: Session, limit: int = 10, household_id: str | None = None):
    try:
        client = get_qdrant_client()
        # Order by created_at so [-5:] / [-3:] pick the most recent swipes
        accepts = session.exec(
            select(SwipeAction)
            .where(SwipeAction.user_id == uuid.UUID(user_id), SwipeAction.action == "accept")
            .order_by(SwipeAction.created_at.asc())  # type: ignore
        ).all()
        ...
        # Household-scoped filter: own recipes + public
        query_filter = None
        if household_id:
            query_filter = models.Filter(
                should=[
        ...
        positive_ids = [str(a.recipe_id) for a in accepts[-5:]]
        negative_ids = [str(r.recipe_id) for r in rejects[-3:]]
...
def update_user_preference(user_id: str, recipe: Recipe, action: str):
    try:
        client = get_qdrant_client()
    ...
    if existing:
        ...
        total = point.payload.get("total_accepts", 0) + point.payload.get("total_rejects", 0)
        weight = 1.0 / (total + 1) if action == "accept" else -0.5 / (total + 1)
        new_vector = [o + weight * (r - o) for o, r in zip(old_vector, recipe_vector)]
        client.upsert(
            collection_name="user_preferences",
            points=[
                models.PointStruct(
                    ...
                    payload={
                        "user_id": user_id,
                        "total_accepts": point.payload.get("total_accepts", 0) + (1 if action == "accept" else 0),
                        "total_rejects": point.payload.get("total_rejects", 0) + (1 if action == "reject" else 0),
                    },
                )
            ],
        )
