...
async def discover_with_context(
    ...
    try:
        ...
        if context_pairs:
            ...
            # client.discover() is not available in the locked qdrant-client version.
            # Use query_points with RecommendQuery built from context pairs,
            # and include the target vector as a prefetch to
            # honor target_text / preference.
            positive_ids = [pair.positive for pair in context_pairs]
            negative_ids = [pair.negative for pair in context_pairs]
