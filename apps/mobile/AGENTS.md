...
| Add/update env var   | **Root** `.env.example` (and root `.env` for local)                                                 | Mobile uses `EXPO_PUBLIC_*` from root when running via `pnpm dev` / `pnpm dev:app` from repo root (or `pnpm dev:metro`). See [.docs/guides/local-dev.md](../../.docs/guides/local-dev.md). |
